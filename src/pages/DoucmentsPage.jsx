...
const DocumentsPage = () => {
    const [files] = useState([
        ...
        { id: "125", title: "Client Meeting Notes - 2025-09-15.docx", date: "Sep 15, 2025" },
    ]);
                        ...
                        {files.map(file => (
                            <div key={file.id} className="grid grid-cols-12 gap-4 items-center p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                                ...
                                <div className="col-span-6 font-medium text-gray-800 truncate">
                                    <Link to={`/dashboard/documents/${file.id}`} className="hover:underline">{file.title}</Link>
                                </div>
                                <div className="col-span-2 text-gray-500 text-sm">No tags</div>
                                <div className="col-span-2 text-gray-500 text-sm">{file.date}</div>
                                <div className="col-span-1 flex items-center justify-end space-x-2">
                                    <Link to={`/dashboard/documents/${file.id}`} className="bg-black text-white text-xs font-semibold px-3 py-1.5 rounded-md">Chat</Link>
